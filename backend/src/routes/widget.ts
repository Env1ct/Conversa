...
import { tenantCors } from '../middleware/auth';
import { db } from '../services/database';

const router = Router();

// Aplicar CORS específico para widgets en todas las rutas
router.use(tenantCors);

// GET /api/widget/:id - Obtener configuración del widget (público)
...
// GET /api/widget/:id/script - Obtener script de widget (público)
router.get('/:id/script', async (req: Request, res: Response) => {
  try {
    const widgetId = req.params.id;

    // Verificar que el widget existe y está activo
    const widget = await db.getClient().widget.findUnique({
      where: { id: widgetId },
      include: {
        tenant: {
...
// GET /api/widget/js/widget.js - Archivo JavaScript del widget (alternativo)
router.get('/js/widget.js', async (req: Request, res: Response) => {
  try {
    // Script alternativo más liviano que se puede cargar dinámicamente
    const widgetJS = `
...
// GET /api/widget/:id/health - Health check del widget específico
